...
const PILLAR_SECTIONS: PillarSection[] = [
    ...
    content: (
      <>
        <p style={{ margin: 0, fontFamily: 'var(--font-body)', fontSize: 'var(--text-body)', color: 'var(--color-text-secondary)', lineHeight: 'var(--leading-relaxed)' }}>
          The quiz maps your civic identity across eight dimensions — real tensions that every thoughtful voter navigates. Layer 1 (roughly 22 questions) establishes your dimensional profile and assigns your Civic Mantle. Layers 2–4 add issue depth, behavioral modifiers, and dealbreakers, raising your completion percentage but not changing your Mantle or constellation.
        </p>
        <p style={{ margin: 0, fontFamily: 'var(--font-body)', fontSize: 'var(--text-body)', color: 'var(--color-text-secondary)', lineHeight: 'var(--leading-relaxed)' }}>
  ...
  },
  {
    ...
    content: (
      <>
        <p style={{ margin: 0, fontFamily: 'var(--font-body)', fontSize: 'var(--text-body)', color: 'var(--color-text-secondary)', lineHeight: 'var(--leading-relaxed)' }}>
          Your Ballot matches you against candidates using the same eight-dimensional model as the quiz. Your profile, importance weights, and dealbreaker filters run against candidate profiles built from public positions, voting records, and stated platforms. Every recommendation includes a plain-English explanation of why each candidate matches or doesn&apos;t. Dealbreaker selections act as hard exclusions — a candidate who crosses one of your lines is removed from your results entirely, regardless of alignment on everything else.
        </p>
        <p style={{ margin: 0, fontFamily: 'var(--font-body)', fontSize: 'var(--text-body)', color: 'var(--color-text-secondary)', lineHeight: 'var(--leading-relaxed)' }}>
          Data sources: congress.gov for federal voting records, FEC openFEC for campaign finance, Open States v3 for state legislative candidates. Candidate profiles are built and reviewed before each election cycle.
        </p>
        <p style={{ margin: 0, fontFamily: 'var(--font-body)', fontSize: 'var(--text-body)', color: 'var(--color-text-secondary)', lineHeight: 'var(--leading-relaxed)' }}>
  ...
  },
  {
    ...
    content: (
      <>
        ...
        <p style={{ margin: 0, fontFamily: 'var(--font-body)', fontSize: 'var(--text-body)', color: 'var(--color-text-secondary)', lineHeight: 'var(--leading-relaxed)' }}>
          <strong style={{ color: 'var(--color-text-primary)' }}>Claude&apos;s role:</strong> The analysis behind these recommendations is generated by Claude, Anthropic&apos;s AI. Claude reads each source&apos;s body of work, scores it on the eight civic dimensions, and drafts the explanation you see on each card. Humans review placements before they go live and can override Claude&apos;s scoring when the evidence warrants it. We also look at thumbs up and thumbs down feedback regularly — when users systematically disagree with a recommendation, that&apos;s a signal we take seriously and investigate. We also use Perplexity to verify current ownership and status of sources in the catalog — my knowledge has a cutoff date, and independent media changes ownership. Current-status verification is a systematic part of our quarterly review.
        </p>
      </>
    ),
  },

...
export default function HowItWorksPage() {
  ...
  const layers = [
    { num: "1", label: "Your values foundation.", desc: "Fifteen questions about what you believe at the level of principle. Closes with your constellation appearing for the first time — a radar chart unique to you across all eight dimensions." },
    ...
    <div style={{ maxWidth: "var(--max-width-content)", margin: "0 auto", padding: "var(--space-16) var(--space-6)" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-12)" }}>
        ...
        <section>
          ...
          <p style={{ fontFamily: "var(--font-body)", fontSize: "var(--text-body)", color: "var(--color-text-secondary)", lineHeight: "var(--leading-relaxed)" }}>Four things built on top of it: <strong style={{ color: "var(--color-text-primary)" }}>Your Ballot</strong> — who&apos;s asking for your vote, matched to your values, every race we can reliably cover. Between elections it becomes <strong style={{ color: "var(--color-text-primary)" }}>Your Officials</strong>: the same match, run against the people already holding the offices. One pillar, two seasons. <strong style={{ color: "var(--color-text-primary)" }}>Beyond Your Ballot</strong> — candidates you can&apos;t vote for, but whose presence in Congress would shape the country. <strong style={{ color: "var(--color-text-primary)" }}>Your Media Diet</strong> — a three-tier recommendation that makes you a smarter citizen. <strong style={{ color: "var(--color-text-primary)" }}>Your Conversations</strong> — Claude-powered preparation for difficult political conversations with people who see things differently. Three ways in: start one you need to have, figure out how to respond to one that's already happening, or rehearse one you've been avoiding. Bridge-building, not debate prep.</p>
        </section>

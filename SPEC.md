...
**Graduated Challenging fallback (all users, 2026-07-06):** If `finalChallenging` is still empty after the standard `topUp` pass, a second pass runs against the full `scoredAll` pool with `tensionOnHeld >= 0.25` (relaxed from 0.40), keeping all other quality floors intact (`reliability >= 75`, `goodFaith === 'high'`, `independence >= 50`). Up to 3 sources are taken, ranked by `tensionOnHeld` descending. The `toppedUp.challenging` flag is set to `true`, triggering the existing disclosure note on the Challenging panel. Rationale: users with centrist profiles — where no dimension deviates far enough from center to generate strong `tensionOnHeld` — currently see an empty Challenging tab. This is the worst outcome for the most important tier. The fallback surfaces the closest available sources, honestly disclosed, rather than leaving the tab empty.
